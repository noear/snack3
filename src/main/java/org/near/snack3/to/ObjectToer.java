package org.near.snack3.to;

import org.near.snack3.ONode;
import org.near.snack3.ONodeData;
import org.near.snack3.OValue;
import org.near.snack3.OValueType;
import org.near.snack3.core.*;
import org.near.snack3.core.exts.EnumWrap;
import org.near.snack3.core.exts.FieldWrap;
import org.near.snack3.core.utils.BeanUtil;
import org.near.snack3.core.utils.StringUtil;
import org.near.snack3.core.utils.TypeUtil;
...
public class ObjectToer implements Toer {
    ...
    public void handle(Context ctx) throws Exception {
        if (null != ctx.node && null != ctx.type) {
            Object rst = analyse(ctx.config, ctx.node, ctx.type, ctx.type);
            if (null != rst) {
                ctx.object = rst;
                ctx.handled = true;
            }
    ...
    public Object analyseArray(Constants cfg, ONodeData d, Class<?> target) throws Exception {
        ...
        if (is(byte[].class, target)) {
            ...
            return val;
        } else if (is(Integer[].class, target) ) {
    ...
    public Object analyseCollection(Constants cfg, ONode o, Class<?> clz, Type type) throws Exception {
        Collection list = TypeUtil.createCollection(clz);
