...
public class OValue {
    ...
    public void set(Object val){
        ...
        if (String.class == (type)) { //string
            setString((String) val);
            return;
        }

        if (Integer.class == (type) || type == Integer.TYPE) { //int
            setInteger((Integer) val);
            return;
        }

        if (Long.class == (type) || type == Long.TYPE) { //long
            setInteger((Long)val);
            return;
        }

        if (Double.class == (type) || type == Double.TYPE) { //double
            setDecimal((Double)val);
            return;
        }

        if (Float.class == (type) || type == Float.TYPE) { //float
            setDecimal((Float)val);
            return;
        }

        if (Boolean.class == (type) || type == Boolean.TYPE) { //bool
            setBool((Boolean)val);
            return;
        }

        if(Date.class == (type)) { //date
            setDate((Date) val);
            return;
        }

        if(BigInteger.class == (type)){ //big interger
            setBignumber((BigInteger)val);
            return;
        }

        if(BigDecimal.class == (type)){ //big decimal
